---
title: "Chrysodeixis chalcites with clustering algorithm"
author: "Randle"
date: "6/23/23"
...
**This is the third time I'm running the analysis for the the data set including human impact. I needed to rerun initially to recover relative contribution values for MAxEnt, but ended up with RIDICULOUSLY high AUCs.  I'm giving this another shot.
...
I previously divided this into training and testing data so I'll just open those up.
```{r B2.Training_testing}
pres_train<-read.csv(file=paste0(genus,"_",species,"_train.csv"))
pres_test<-read.csv(file=paste0(genus,"_",species,"_train.csv"))
```
...
env_data<-extract(predictors,sdmdataframe)
...
NClayers
```
So far so good.  These are the same predictors that we've had in previous runs.

## Background Data
We need background data for all three models. We generate background data to gauge the probability of environmental conditions, and therefore it is best to do this within the range of occurrences. This is trivial if the occurrence points all occur in one well defined range. However, if occurrences are clumpy...occupying disjunct ranges, we don't want to sample all of the area in between the clumps in which there are no occurrences.
We'll use a method of sums of squares to determine what the optimal number of clusters is.
```{r _sum-of-squares_cluster_ID}
...
```
We want to pick out the smallest number of clusters possible that will give us the smallest sum of squares. This is an exercise in eyeballing. You'll want to pick the smallest number of clusters possible to minimize sum of squares. This should be where the curve first flattens out. 
```{r 6_nclusters}
nclusters<-6
```
...
```
Let's see if these fit in the polygons. **Hash out the lines that are greater than the number of clusters you have. For example, I have hashed out the line `points(abs_raw[[8]], col='purple', cex=0.2)`.**
...
points(abs_raw[[3]], col='green', cex=0.2)
points(abs_raw[[4]], col='purple', cex=0.2)
points(abs_raw[[5]], col='pink', cex=0.2)
points(abs_raw[[6]], col='yellow', cex=0.2)
#points(abs_raw[[7]], col='orange', cex=0.2)
#points(abs_raw[[8]], col='gray', cex=0.2)
```
OK...now I need to remove all points that aren't associated with environmental data. Let's sample environmental data, make an index of complete cases and then sample those cases from abs_raw. Again, hash out lines for non-existent clusters that you don't want to plot.
...
points(abs_train[[3]], col='green', cex=0.2)
points(abs_train[[4]], col='purple', cex=0.2)
points(abs_train[[5]], col='pink', cex=0.2)
points(abs_train[[6]], col='yellow', cex=0.2)
#points(abs_train[[7]], col='orange', cex=0.2)
#points(abs_raw[[8]], col='gray', cex=0.2)
```
...
plot(spol, add=TRUE)
box()
points(abs_train_random)
```


## General additive model
# Data preparation
Generally speaking, we want to sample absence data from the region in which the presence data occur. Now that we have generated a random data set of points, let's sample a number of points equal to the number of presence training points that we have.

```{r}
#convert presence training data into a spatial points dataframe.
abs_trainGAM <- abs_train_random[1:nrow(pres_train),]
...
```{r}
abs_train_data<-extract(predictors,abs_trainGAM)
...
```{r}
gam <- gam(pa ~  Isothermality + Temp_Seasonality + Mean_temp_wettest_q +    
Ann_.precip + Precip_driest_week + Precip_wettest_week + Precip_Seasonality + Hghest_weekly_rad + Rad_seasonality + Rad_wettest_q + Rad_driest_q + Mean_moisture_coldest_q + Elev + human_impact,                      
...
```{r}
background_train <- abs_train_random[1:10000,]
  #You'll get a warning saying that your CRS object has lost a comment. This is unimportant and can be ignored.
```
...
writeRaster(MEpreds, filename=paste0(genus,"_",species,"_ME.tif"))
#and plot
...
First we'll use the pres_test data to generate absence test data. This time we want about the same number of points for both. *If you have a tone of data points, you can sample from any point within that data frame, i.e. `abs_test <- abs_train_random[10000:(10000+length(pres_test)),]` if we want to start sampling at 10,000
```{r}
abs_test <- abs_train_random[1:nrow(pres_test),]
```
